*** Settings ***
Documentation     MT Database related keywords using class implementation
Library           String
Library           Collections

#move this library import outside 
#Library           MTLibrary 
...
Parameter ${parameter no} and parameter price ${combo value} the parameter price name should be ${combo string}
    [Documentation]         Checks the parameter price name given a value
    Log Variables       trace
    ${actual} =                   Get Combo Box Value Attribute Using Value    ${parameter no}   ${combo value}    parameter_price_name
    Log Parameter Info      ${parameter no}
    Should be equal as strings    ${actual}    ${combo string}
